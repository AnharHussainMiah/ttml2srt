    ...
    match self::to_str(&args.file_name) {
        Ok(srt) => {
            match std::fs::File::create(format!("{}", args.file_name.replace(".ttml", ".srt"))) {
                Ok(mut w) => {
                    for line in &srt {
                        writeln!(&mut w, "{}", line).expect(&format!("ERROR: unable to write line `{}`", line));
                    }
                    println!("success! created SRT file `{}` wrote [{}] lines", args.file_name.replace(".ttml", ".srt"), srt.len());
                },
...
fn to_str(f: &str) -> Result<Vec<String>, String> {
    ...
    let content = std::fs::read_to_string(f).map_err(|_| format!("unable to open file `{}` are you sure this exists or spelt correctly?", f))?;
    let element: Element = content.parse().map_err(|_| format!("unable to parse file `{}`", f))?;
    let ns = "http://www.w3.org/ns/ttml";

    let body = element.get_child("body", ns).ok_or("unable to extract `body` element");
    let div = body?.get_child("div", ns).ok_or("unable to extract `div` element");
    let ps = div?.children();
    ...
    for child in ps {
        if let (Some(begin), Some(end)) = (child.attr("begin"), child.attr("end")) {
            ...
            for subs in child.children() {
                if subs.text() != "" {
                    srt.push(format!("{}", subs.text().replace("\n", "").replace("\t", "")))
                }
            }
            srt.push(format!(""));
        }
